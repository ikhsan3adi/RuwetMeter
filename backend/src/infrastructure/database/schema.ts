...
export const contentTypeEnum = pgEnum('content_type', ['raw', 'cleaned', 'summary'])
...
export const newsArticles = pgTable(
  'news_articles',
  {
    ...
    scoreEconomy: integer('score_economy').notNull().default(0),
    scorePolitics: integer('score_politics').notNull().default(0),
    scoreInfrastructure: integer('score_infrastructure').notNull().default(0),
    scoreSocial: integer('score_social').notNull().default(0),
...
export const ruwetLogArticles = pgTable(
  ...
  (table) => [primaryKey({ columns: [table.logId, table.articleId] })],
)
