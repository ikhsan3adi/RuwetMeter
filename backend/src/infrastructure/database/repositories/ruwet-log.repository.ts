...
function rowToRuwetScore(row: typeof ruwetLogs.$inferSelect, articleIds: string[]): RuwetScore {
  return {
    id: row.id,
...
export class RuwetLogRepository implements RuwetLogRepositoryPort {
  ...
  async getLatest(): Promise<RuwetScore | null> {
    const [row] = await db.select().from(ruwetLogs).orderBy(desc(ruwetLogs.createdAt)).limit(1)
  ...
  async getHistory(days: number): Promise<RuwetScore[]> {
    ...
    return logRows.map((row) => rowToRuwetScore(row, articleMap.get(row.id) ?? []))
  }
