...
import { OpenAIAnalysisAdapter, OpenAIChatAdapter } from './openai.adapter'
...
export class OpencodeAnalysisAdapter implements AnalysisProviderPort {
  ...
  constructor(model?: string) {
    this.inner = new OpenAIAnalysisAdapter(BASE_URL, config.OPENCODE_API_KEY, model ?? 'big-pickle')
  }
...
export class OpencodeChatAdapter implements ChatProviderPort {
  ...
  constructor(model?: string) {
    this.inner = new OpenAIChatAdapter(BASE_URL, config.OPENCODE_API_KEY, model ?? 'big-pickle')
  }
