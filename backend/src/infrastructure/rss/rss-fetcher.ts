...
export class RssFetcher implements RssFetcherPort {
  ...
  private async parseFeed(url: string): Promise<RssItem[]> {
    ...
    if (rssChannel?.item) {
      const feedItems = Array.isArray(rssChannel.item) ? rssChannel.item : [rssChannel.item]
    ...
    if (atomFeed?.entry) {
      const feedItems = Array.isArray(atomFeed.entry) ? atomFeed.entry : [atomFeed.entry]
      for (const el of feedItems) {
        const title = el.title ?? ''
        const link = (typeof el.link === 'string' ? el.link : el.link?.['@_href']) ?? ''
        const pubDate = el.published ?? el.updated ?? ''
        if (title && link) items.push({ title, link, pubDate, source })
      }
    }
  ...
  private async scrapeArticle(url: string): Promise<string> {
    const res = await fetch(url, {
      signal: AbortSignal.timeout(15000),
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; RuwetMeter/1.0; +https://ruwetmeter.app)',
      },
    })
